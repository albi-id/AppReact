...
app.post('/driver/profile', authenticate, async (req, res) => {
...
app.patch('/driver/availability', authenticate, async (req, res) => {
...
app.patch('/driver/location', authenticate, async (req, res) => {
...
app.post('/services/request', authenticate, async (req, res) => {
...
app.post('/services/match', authenticate, async (req, res) => {
    ...
    try {
        ...
        const getDistanceFallback = (lat1, lon1, lat2, lon2) => {
            const R = 6371;
        ...
        const candidates = drivers
            ...
            if (!loc)
                return { driver, distanceKm: Infinity, etaMinutes: null };
...
app.patch('/services/:serviceId/reject', authenticate, async (req, res) => {
    ...
    try {
        ...
        const candidates = drivers
            ...
            if (!loc)
                return { driver: d, distance: Infinity };
            const distance = getDistance(service.pickupLat, service.pickupLng, loc.lat, loc.lng);
...
app.get('/services/:serviceId/eta', authenticate, async (req, res) => {
    const { serviceId } = req.params;
    try {
        const service = await prisma.service.findUnique({
            where: { id: serviceId },
        ...
        const driverLoc = service.driver.driverProfile.lastLocation;
        if (!driverLoc) {
            return res.json({
        ...
        if (etaMinutes === null) {
            ...
            distanceKm = getDistance(service.pickupLat, service.pickupLng, driverLoc.lat, driverLoc.lng);
...
app.delete('/services/:serviceId', authenticate, async (req, res) => {
    const { serviceId } = req.params;
    try {
        const service = await prisma.service.findUnique({
            where: { id: serviceId },
...
app.patch('/services/:serviceId/arrive', authenticate, async (req, res) => {
    const { serviceId } = req.params;
    try {
...
app.patch('/services/:serviceId/finish-wait', authenticate, async (req, res) => {
    const { serviceId } = req.params;
    try {
        if (req.dbUser.role !== 'DRIVER') {
        ...
        const waitDurationMinutes = Math.round((waitEndAt.getTime() - service.arrivedAt.getTime()) / 1000 / 60);
        ...
        const tarifaMinuto = tarifasPorMinuto[service.type] ?? 0;
...
app.patch('/services/:serviceId/pay-cash', authenticate, async (req, res) => {
    const { serviceId } = req.params;
    try {
        const service = await prisma.service.findUnique({
            where: { id: serviceId },
...
app.patch('/services/:serviceId/pay-wallet', authenticate, async (req, res) => {
    const { serviceId } = req.params;
    try {
        ...
        console.log(`Email simulado enviado a ${service.requester?.email || 'solicitante'}: Pago por billetera confirmado por ${service.amount} ARS`);
        res.json({
...
app.patch('/services/:serviceId/confirm-payment', authenticate, async (req, res) => {
    const { serviceId } = req.params;
    try {
        const service = await prisma.service.findUnique({
            where: { id: serviceId },
        ...
        console.log(`[NOTIFICACIÓN APP] Solicitante ${service.requester?.email || 'desconocido'}: Pago confirmado por ${service.amount} ARS (${service.paymentMethod})`);
        // Email simulado (más detallado)
        console.log(`Email simulado enviado a ${service.requester?.email || 'solicitante'}: 
      Pago confirmado!
...
app.get('/admin/services', authenticate, async (req, res) => {
    try {
...
app.get('/admin/drivers', authenticate, async (req, res) => {
    try {
        if (req.dbUser.role !== 'ADMIN') {
...
app.get('/services/my', authenticate, async (req, res) => {
    try {
        const services = await prisma.service.findMany({
            where: {
                requesterId: req.user.id,
...
app.get('/services/driver/my', authenticate, async (req, res) => {
    try {
        if (req.dbUser.role !== 'DRIVER') {
...
app.patch('/services/:serviceId/accept', authenticate, async (req, res) => {
    const { serviceId } = req.params;
    try {
        if (req.dbUser.role !== 'DRIVER') {
